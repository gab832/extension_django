...
class Paginator:
    ...
    def paginate_queryset(self, queryset, model):
    ...
    async def apaginate_queryset(self, queryset, model):
        self.extract_limit_offset_from_rql()
        self.limit = self.sanitize_limit()
...
def RQLLimitOffsetPaginator(
    ...
    def _wrapper(
        ...
        return Paginator(
            unquote(request.scope['query_string'].decode()),
            default_limit,
        )
    return Depends(_wrapper)


def model_not_found_exception_handler(request: Request, exc: ObjectDoesNotExist):
    return JSONResponse(status_code=404, content='Recipe Object not found.')

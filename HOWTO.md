# Welcome to Connect Extension project fastapi django

This is a MVP project that demonstrates the usage of our current extensions with django ORM and filters.


## Next steps

You may open your favourite IDE and start working with your project, please note that this project runs using docker.
You may modify at any time the credentials used to authenticate to connect modifying the file *fastapi_django/.fastapi_django_dev.env*.
...
```sh
$ docker compose up fastapi_django_dev
```
...
```sh
$ docker compose run fastapi_django_bash
```
...
```sh
$ docker compose run fastapi_django_test
```
